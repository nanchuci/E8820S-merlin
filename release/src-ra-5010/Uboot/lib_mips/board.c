...
#if defined(ASUS_RPAC87) || defined(ASUS_RTAC85U) || defined(ASUS_RTAC85P) || defined(ASUS_RTN800HP) || defined (ASUS_RTACRH26) || defined (ASUS_RMAC2100) || defined (ASUS_RTE8820S)
...
#if defined(ASUS_RTN56UB1) || defined(ASUS_RTAC1200GA1) || defined(ASUS_RTAC1200GU)
	///turn off LEDs at assemble code ,except PWR led 
	value = le32_to_cpu(*(volatile u_long *)(RALINK_PIO_BASE + 0x20));
	value |= (0xf<<13)|(0x1<<7);
	*(volatile u_long *)(RALINK_PIO_BASE+ 0x20) = cpu_to_le32(value);	
#endif	
...
#if defined(SWITCH_CHIP_RTL8367RB)
int rtl8367r_switch_inited = 0;
#endif
...
/*config bootdelay via environment parameter: bootdelay */
	...
	if (timer1 <= CONFIG_BOOTDELAY)
       ...
       mtk7620_set_gpio_pin(WIFI_2G_LED, 1);
       mtk7620_set_gpio_pin(USB_LED, 1);
#elif defined(ASUS_RTN56UB1) || defined(ASUS_RTAC1200GA1) || defined(ASUS_RTAC1200GU)
	...
	mtk7621_set_gpio_pin(WIFI_2G_LED, 1);
	mtk7621_set_gpio_pin(WIFI_5G_LED, 1);
#elif defined(ASUS_RTAC85P) || defined (ASUS_RTACRH26) || defined (ASUS_RMAC2100) || defined (ASUS_RTE8820S)
	/* turn on PWR, turn off WAN, LAN, USB, WPS, 2G, 5G LED */
	mtk7621_set_gpio_pin(PWR_LED, 0);
	mtk7621_set_gpio_pin(WIFI_2G_LED, 1);
	mtk7621_set_gpio_pin(WIFI_5G_LED, 1);
...
#elif defined (MT7621_ASIC_BOARD)
	mtk7621_set_gpio_pin(ALL_LED_OFF_GPIO_NR, 1);	/* Inhibit ALL LED, except PWR LED */
#else
    set_gpio_pin(ALL_LED_OFF_GPIO_NR, 1);	/* Inhibit ALL LED, except PWR LED */
#endif	
	...
	int ii = rtl8367r_switch_init_pre();
	printf("rtl8367r_switch_init_pre() return %d\n", ii);
#endif
